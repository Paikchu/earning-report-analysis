...
代码已按 Web / Pipeline 所有权拆分；最新目录与扩展路线见 [架构文档](service-architecture.md)。
...
| Shared (`shared/analysis-contract/`) | Backend-only |
|---|---|
| Wire types, error codes and scopes | `workers/pipeline/read-api/**` (router, auth) |
| JSON Schemas + the OpenAPI document | `workers/pipeline/src/sec/public-api.ts`, `workers/pipeline/src/fundamentals/fundamentals-api.ts`, `workers/pipeline/src/company-analysis/api.ts` |
| `AnalysisBackendClient` (server-only) | `workers/pipeline/src/sec/d1.ts`, `workers/pipeline/src/fundamentals/fundamentals-d1.ts`, `workers/pipeline/src/company-analysis/repository.ts` |
| — | `workers/pipeline/src/db/schema.ts`, `workers/pipeline/src/db/fundamentals-schema.ts`, `workers/pipeline/migrations/**` |
...
- Never put a read credential in a `var`, in `wrangler.jsonc`, in a `NEXT_PUBLIC_*` variable, or in
  ...
  `lib/web/analysis-backend-runtime.ts`, which is server-only.
