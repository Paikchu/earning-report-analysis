...
| Web → Pipeline `PIPELINE` Service Binding | Confirmed (`lib/web/sec-runtime.ts`, `lib/web/service-binding.ts`). |
...
| `app/stocks/[ticker]/SecFilingsSection.tsx` (client) | browser `fetch('/api/v1/.../filings')` | unchanged URL; type import moves to shared contract |
...
`@/db`, `workers/pipeline/src/sec/d1.ts`, `workers/pipeline/src/fundamentals/fundamentals-d1.ts`, `workers/pipeline/src/company-analysis/repository.ts`
and the query modules that construct them (`workers/pipeline/src/sec/public-api.ts`,
`workers/pipeline/src/fundamentals/fundamentals-api.ts`, `workers/pipeline/src/company-analysis/api.ts`). All become backend-only;
the wire types they exported move to `shared/analysis-contract/`.

### 1.2 Existing defects / hazards found

| ID | Finding |
|---|---|
| F1 | `D1SecRepository.getPublishedReport` wraps its query in `try { … } catch { return null }`, and `workers/pipeline/src/sec/feed.ts` adds `.catch(() => null)`. A D1 outage is therefore reported to readers as "this filing has no analysis" — an infrastructure failure returned as an empty success (prompt §4.2). |
...
| B5 | Web loses its `DB` D1 binding entirely. The database holds only analysis tables (verified against `workers/pipeline/src/db/schema.ts` + `workers/pipeline/src/db/fundamentals-schema.ts`), so no unrelated website feature depends on it. | §Phase 4. |
...
| B9 | Dead code removed: `handleSecFeedRequest` + `buildSecWatchlist` (`lib/web/sec-api.ts`), `getCachedSecFeed` (`workers/pipeline/src/sec/feed.ts`), `db/index.ts`, and the `cloudflareArtifacts` Vite plugin. | The first pair was the last storage-touching import reachable from the admin routes; the rest were orphaned by the boundary move. |

## 3. Work items

| ID | Item | Status | Evidence |
|---|---|---|---|
| W01 | Shared runtime-neutral contract module `shared/analysis-contract/` | done | `tests/analysis-contract.test.ts` |
