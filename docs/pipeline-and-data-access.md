...
### 2.1 入口（`workers/pipeline/src/index.ts`）
...
每个 Workflow 的重试策略统一（`WORKFLOW_RETRY`，见 [`workflow-core.ts`](../workers/pipeline/src/workflow-core.ts)）：单个 `step.do` 最多重试 3 次，延迟 30s / 90s / 180s（±20% 抖动），超时 5 分钟。**从第 2 次尝试起，模型调用会自动切换到备用模型 `hy3`**（`retry-policy.ts`），只有到最后一次尝试仍失败，对应的分析节点才会被标记为失败节点而不是让整篇 filing 直接丢失。
...
1. 每一轮：先直接调 `syncFundamentals(env.DB, ticker)`（[`fundamentals.ts`](../workers/pipeline/src/fundamentals.ts)，内部是 `FundamentalSyncService` 抓 Yahoo + 写 D1，跟 Pipeline 自己 Cron 里定期跑的是同一个函数），再用 `buildCompanyAnalysisPacket` 直接查 D1 读一次「当期 packet」是否 `ready`。这一步以前是桥接调用 Web Worker 的 `/api/internal/fundamentals/refresh`（该 Worker 再用 `waitUntil` 异步抓取，失败只能进日志）；现在 Yahoo 抓取本身就是一个 durable step，失败由 Workflow 重试，调用方也会真正等到写完成，不再是"发了就不管"。
...
5. `company-agent-*` 系列 step 跑多轮模型 Agent（[`company-analysis-agent.ts`](../workers/pipeline/src/company-analysis-agent.ts)），每一次模型交互都是独立的 durable step。
...
**按需刷新这条路径已经删除。** 读取 fundamentals 不再触发任何刷新——那是一次藏在读请求里的写操作，而且频率由"有多少人打开页面"决定。刷新改由两处负责：Pipeline 自己的 Cron 巡检（[`fundamentals-sweep.ts`](../workers/pipeline/src/fundamentals-sweep.ts)，只覆盖白名单里超过 24 小时未成功抓取的股票，每次最多 2 个），以及上表最后一行那个受认证的端点。
...
- `metrics` 传未知的 `metricKey` 会返回 `400 { "code": "INVALID_METRICS" }`；合法取值见 [`workers/pipeline/src/fundamentals/fundamental-metrics.ts`](../workers/pipeline/src/fundamentals/fundamental-metrics.ts) 里的 `FUNDAMENTAL_METRIC_CATALOG`。
...
`filings` / `filings/:accession` 里的 `analysis` 字段就是 Pipeline 发布时写入 D1 的完整研报（`SecAnalysisArtifact`，定义见 [`workers/pipeline/src/sec/types.ts`](../workers/pipeline/src/sec/types.ts)），顶层结构：
...
对消费方最重要的是 `report.dataQuality`：`verificationStatus`（`verified` 才代表完全通过门禁）、`analysisStatus`、`unresolvedQuestions`、`failedNodeIds`、`stopReason`。展示这份研报时**应该原样展示或至少标注 `dataQuality`**，不要把 `partial` 状态的内容当成和 `complete` 一样可信来用。完整字段定义见 [`workers/pipeline/src/sec/analysis.ts`](../workers/pipeline/src/sec/analysis.ts)。
