...
function env(started: Array<{ ticker: string; backfill?: boolean }>, trackedTickers: string): SecCronEnv {
  return { SEC_TRACKED_TICKERS: trackedTickers, SEC_REFRESH_KEY: "secret", SEC_ANALYSIS_WORKFLOW: binding(started) };
}

test("Cron reads its own whitelist and normalizes it", async () => {
  const started: Array<{ ticker: string; backfill?: boolean }> = [];

  assert.deepEqual(await runSecRefresh(env(started, " msft, MSFT, NVDA ")), { started: ["MSFT", "NVDA"], failed: [] });
...
test("a manual job answers 503 when the whitelist is misconfigured", async () => {
  const started: Array<{ ticker: string; backfill?: boolean }> = [];
