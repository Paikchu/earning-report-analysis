...
test("renders the complete report, dynamic evidence, and analysis completeness", () => {
  const filing: SecFilingWithSummary = {
    ...
    analysis: {
      ...
      dataQuality: { coverage: 1, verificationStatus: "verified", warnings: [], analysisStatus: "partial", stopReason: "max_rounds", unresolvedQuestions: ["cloud-growth"], failedNodeIds: ["margin-bridge"], managerCoverageScore: 0.8 },
    },
  };
  ...
  assert.match(html, /部分完成 · 修复轮次用尽/);
  assert.match(html, /主编覆盖度/);
  assert.match(html, /未解决问题：cloud-growth/);
