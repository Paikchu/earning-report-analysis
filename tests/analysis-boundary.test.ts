 ...
 */
const BACKEND_ONLY = [
  "workers/pipeline/src/sec/d1.ts",
  "workers/pipeline/src/sec/public-api.ts",
  "workers/pipeline/src/sec/feed.ts",
  "workers/pipeline/src/sec/pipeline.ts",
  "lib/web/sec-runtime.ts",
  "workers/pipeline/src/fundamentals/fundamentals-d1.ts",
  "workers/pipeline/src/fundamentals/fundamentals-api.ts",
  "workers/pipeline/src/fundamentals/fundamental-sync.ts",
  "workers/pipeline/src/fundamentals/yahoo-fundamentals-client.ts",
  "workers/pipeline/src/company-analysis/repository.ts",
  "workers/pipeline/src/company-analysis/api.ts",
  "workers/pipeline/src/company-analysis/packet.ts",
  "workers/pipeline/src/company-analysis/feature-engine.ts",
  "workers/pipeline/src/db/schema.ts",
  "workers/pipeline/src/db/fundamentals-schema.ts",
];
...
test("the shared contract module imports nothing that reaches storage or a framework", async () => {
  const contractFiles = (await readdir(join(ROOT, "shared/analysis-contract")))
    .filter((entry) => entry.endsWith(".ts"))
    .map((entry) => join("shared/analysis-contract", entry));
...
test("every backend Workflow class is still exported from the Worker entry point", async () => {
  const entry = await readFile(join(ROOT, "workers/pipeline/src/index.ts"), "utf8");
  ...
  const handler = await readFile(join(ROOT, "workers/pipeline/src/worker.ts"), "utf8");
...
test("the boundary walker detects a real violation when there is one", async () => {
  const chains = await reachableBackendModules("workers/pipeline/src/index.ts", new Set());
  ...
  assert.ok(
    flattened.some((chain) => chain.includes("workers/pipeline/src/sec/d1.ts")),
  ...
  assert.equal(resolveSpecifier("app/page.tsx", "../workers/pipeline/src/sec/config.ts"), "workers/pipeline/src/sec/config.ts");
  assert.equal(resolveSpecifier("workers/pipeline/src/index.ts", "./core.ts"), "workers/pipeline/src/core.ts");
